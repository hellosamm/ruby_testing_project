...
describe Dog do
  ...
  subject(:max) {described_class.new("Max", "Doodle", "Blonde")}
  # Write a test using the second shared_example to test that dog responds to
  # talk ('WOOF!').
  context 'when Dog has a method name shared with another class' do
    include_examples "shared method name"
  end

  # remove the 'x' before running this test
  it 'is not barking' do
    expect(max).to_not be_barking
  end

  # remove the 'x' before running this test
  it 'is sleeping' do
    expect(max).to be_sleeping
  end
end
